...
# === Choice of scenario =====
np.random.seed(8) # 15 : good overtaking # 8 : collision in overtaking
...
for i in range(X_robot0.shape[0]):
    ...
    for j in range(2):
        ...
        ttc_cov_norm1[j][i] = ttc_cov(np.array([X_robot0[i,0], X_robot0[i,1]]), w,h, X_robot0[i,2], X_robot0[i,3], X_robot0[i,4],
                np.array([X_robot[i,0], X_robot[i,1]]), w,h, X_robot[i,2], X_robot[i,3], 0.0, cov, ax, 0.997) # 3sigma
        ...
        ttc_cov_norm3[j][i] = ttc(np.array([X_robot0[i,0], X_robot0[i,1]]), w,h, X_robot0[i,2], X_robot0[i,3], X_robot0[i,4],
...
ax1.plot( np.arange(X_robot0.shape[0]), ttc_cov_norm1[0], color=sns.color_palette()[0], label="ttc_cov 3sigma")
ax1.plot( np.arange(X_robot0.shape[0]), ttc_cov_norm2[0], color=sns.color_palette()[1], label="ttc_samples")
ax1.plot( np.arange(X_robot0.shape[0]), ttc_cov_norm3[0], color=sns.color_palette()[2], label="ttc")

ax2.plot( np.arange(X_robot0.shape[0]), ttc_cov_norm1[1], color=sns.color_palette()[0], label="ttc_cov 3sigma")
ax2.plot( np.arange(X_robot0.shape[0]), ttc_cov_norm2[1], color=sns.color_palette()[1], label="ttc_samples")
ax2.plot( np.arange(X_robot0.shape[0]), ttc_cov_norm3[1], color=sns.color_palette()[2], label="ttc")
...
if collision:
    if j+1 == 1:
        ax1.axvline(x=ttc_real, color='k', linestyle='--', label="Collision detected")
    else:
        ax2.axvline(x=ttc_real, color='k', linestyle='--', label="Collision detected")
ax1.legend()
ax2.legend()
plt.grid(True)
plt.show()